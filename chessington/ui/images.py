...
class ImageRepository:
    ...
    def _load_from_disk(self):
        ...
        for piece in self.PIECES_WITH_IMAGES:
            mapping[piece] = {}
            mapping[piece][Player.WHITE] = piece_on_all_backgrounds(
                piece(Player.WHITE))
            mapping[piece][Player.BLACK] = piece_on_all_backgrounds(
                piece(Player.BLACK))
...
def get_filename_for_piece(piece: Optional[Piece]) -> str:
    ...
    image_name = piece.__class__.__name__.lower(
    ) + piece.player._name_.lower()[0] + '.png'
